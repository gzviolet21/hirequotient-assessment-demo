...
const nusaSipQuestions = [
  ...
  },
  {
    ...
    type: "single",
    prompt: "Which statement about NusaSip's Year 1 → Year 2 P&L is correct?",
    instruction: "Use Exhibit 1. Select the best-supported statement.",
    options: [
      ["A", "Profit fell from IDR 40.0bn to IDR 35.2bn because operating costs grew faster than revenue (21.75% versus 15%)."],
      ["B", "Profit grew alongside revenue, since revenue rose 15% year over year."],
      ["C", "Profit stayed flat because revenue and costs grew at the same rate."],
      ["D", "The profit decline is a one-off event unrelated to the core business, since revenue still grew 15%."],
    ],
    answer: "A",
    points: 10,
    exhibit: {
  ...
  },
  {
    ...
    category: "Quantitative reasoning",
    type: "number",
    prompt: "NusaSip values BrewPack at 8× annual profit impact. Profit impact equals BrewPack's IDR 7.0bn standalone annual profit plus the combined Year-1 manufacturing synergy across both RTD lines in Exhibit 4 — for each line, (market price − raw material cost − energy cost − shared depreciation − shared manpower) × volume shifted to BrewPack. What is NusaSip's maximum financial bid? Enter IDR billion, to one decimal place.",
    instruction: "Enter your response as a number to one decimal place.",
    answer: 120.8,
    tolerance: 0.05,
    points: 14,
    exhibit: {
      title: "Exhibit 4 · Acquisition valuation inputs by RTD line",
      note: "Year 1, IDR per bottle unless noted",
      columns: ["Input", "RTD Coffee", "RTD Tea"],
      rows: [
        ["Volume shifted to BrewPack (m bottles)", "6.0", "3.0"],
        ["Market price per bottle", "4,800", "4,800"],
        ["Raw material cost per bottle", "2,000", "2,200"],
        ["Energy cost per bottle", "500", "600"],
        ["Shared depreciation per bottle", "700", "700"],
        ["Shared manpower per bottle", "600", "600"],
      ],
    },
  },
  {
  ...
  },
  {
    ...
    type: "multi",
    selectMode: "open",
    prompt: "Which risks should NusaSip prioritize before acquiring BrewPack?",
    instruction: "Select all the risks that are material and specific to this decision.",
    options: [
...
const bumiBiteQuestions = [
  ...
  { id: "profit", category: "Data interpretation", type: "single", prompt: "Which statement about BumiBite's Year 1 → Year 2 P&L is correct?", instruction: "Use Exhibit 1. Select the best-supported statement.", options: [["A", "Profit fell from IDR 32.0bn to IDR 30.4bn because operating costs grew faster than revenue (20.0% versus 15.0%)."], ["B", "Profit grew alongside revenue, since revenue rose 15% year over year."], ["C", "Profit stayed flat because revenue and costs grew at the same rate."], ["D", "The profit decline is a one-off event unrelated to the core business, since revenue still grew 15%."]], answer: "A", points: 10, exhibit: { title: "Exhibit 1 · BumiBite group P&L", note: "IDR billion", columns: ["Metric", "Year 1", "Year 2"], rows: [["Revenue", "160.0", "184.0"], ["Total operating costs", "128.0", "153.6"]] } },
  ...
  { id: "valuation", category: "Quantitative reasoning", type: "number", prompt: "BumiBite values NutriPak at 8× annual profit impact. Profit impact equals NutriPak's IDR 5.5bn standalone annual profit plus the combined Year-1 manufacturing synergy across both SKUs in Exhibit 4 — for each SKU, (market price − raw material cost − energy cost − shared depreciation − shared manpower) × volume shifted to NutriPak. What is BumiBite's maximum financial bid? Enter IDR billion, to one decimal place.", instruction: "Enter your response as a number to one decimal place.", answer: 92.0, tolerance: 0.05, points: 14, exhibit: { title: "Exhibit 4 · Acquisition valuation inputs by SKU", note: "Year 1, IDR per pack unless noted", columns: ["Input", "Original", "Spicy"], rows: [["Volume shifted to NutriPak (m packs)", "6.0", "4.0"], ["Market price per pack", "3,900", "3,900"], ["Raw material cost per pack", "1,600", "1,800"], ["Energy cost per pack", "500", "550"], ["Shared depreciation per pack", "600", "600"], ["Shared manpower per pack", "500", "500"]] } },
...
const suryaGridQuestions = [
  ...
  { id: "score", category: "Data interpretation", type: "single", prompt: "SuryaGrid weights emissions 50%, reliability 25%, affordability 15%, and execution readiness 10%. Which portfolio scores higher on the weighted scorecard, and what does that imply?", instruction: "Use the scores in Exhibit 3 and the budget and tariff guardrails from Exhibit 2.", options: [["A", "Portfolio A scores higher (88.0 vs 86.75), but it breaches the IDR 170bn budget and +0.6% tariff guardrails, so the scorecard ranking should not override the constraint-feasible choice, Portfolio B."], ["B", "Portfolio B scores higher (88.0 vs 86.75), so it should be chosen on scorecard grounds alone."], ["C", "Portfolio A and Portfolio B score identically once the weights are applied."], ["D", "Portfolio A should be chosen because it scores highest, regardless of budget or tariff feasibility."]], answer: "A", points: 12, exhibit: { title: "Exhibit 3 · Portfolio scorecard", note: "0 = weakest, 100 = strongest", columns: ["Portfolio", "Emissions", "Reliability", "Affordability", "Execution readiness"], rows: [["A", "100", "100", "40", "70"], ["B", "90", "80", "85", "90"]] } },
  { id: "risks", category: "Business judgment", type: "multi", selectMode: "open", prompt: "Which risks deserve explicit mitigation before SuryaGrid commits to the transition portfolio?", instruction: "Select all the risks that are material and specific to this plan.", options: [["A", "Land, permitting, or grid-connection delays could push back solar delivery."], ["B", "Demand-response participation could be lower than expected."], ["C", "Diesel retirement could impair reliability if replacement capacity is late."], ["D", "The company might receive fewer likes on a transition announcement."], ["E", "Office coffee demand could change during construction."], ["F", "The company logo may look less modern than a competitor's."]], answer: ["A", "B", "C"], points: 10, exhibit: null },
...
const healthAccessQuestions = [
  ...
  { id:"score",category:"Data interpretation",type:"single",prompt:"ArchipelagoCare weights access 45%, equity 30%, quality 15%, and cost 10%. Which portfolio scores higher on the weighted scorecard, primarily because of which criterion?",instruction:"Use Exhibit 3.",options:[["A","Portfolio A scores higher (92.5 vs 79.25), primarily because of its Access score — it fully reaches the 100,000-consultation target, while Portfolio B reaches only about 80,000."],["B","Portfolio B scores higher, primarily because of its lower cost."],["C","Portfolio A and Portfolio B score identically once the weights are applied."],["D","Portfolio A scores higher, primarily because of its Quality score."]],answer:"A",points:12,exhibit:{title:"Exhibit 3 · Portfolio scorecard",note:"0 = weakest, 100 = strongest",columns:["Portfolio","Access","Equity","Quality","Cost"],rows:[["A","100","90","90","70"],["B","80","75","85","80"]]} },
  { id:"risks",category:"Business judgment",type:"multi",selectMode:"open",prompt:"Which risks require mitigation before rollout?",instruction:"Select all the risks that are material and specific to this decision.",options:[["A","Digital triage could exclude patients with poor connectivity"],["B","Mobile clinics may struggle to retain qualified staff"],["C","Higher volume could weaken follow-up care if referrals are not integrated"],["D","Telehealth subscription costs could pressure ArchipelagoCare's existing IT budget"],["E","Cafeteria menus may change"],["F","Patient appointment no-show rates could remain unchanged after the rollout"]],answer:["A","B","C"],points:10,exhibit:null },
...
const circularPackQuestions = [
  ...
  { id:"score",category:"Data interpretation",type:"single",prompt:"PacificPack weights material reduction 40%, consumer adoption 25%, product quality 20%, and execution readiness 15%. Which portfolio scores higher, and why?",instruction:"Use Exhibit 3.",options:[["A","Portfolio A scores higher (88.8 vs 79.0); despite a similar reduction figure, Portfolio B's compostable-film component weakens shelf life and consumer adoption, pulling its score down."],["B","Portfolio B scores higher because compostable film achieves a larger plastic reduction."],["C","Portfolio A and Portfolio B score identically once the weights are applied."],["D","Portfolio B scores higher because of its stronger execution readiness."]],answer:"A",points:12,exhibit:{title:"Exhibit 3 · Portfolio scorecard",note:"0 = weakest, 100 = strongest",columns:["Portfolio","Reduction","Adoption","Quality","Readiness"],rows:[["A","100","70","100","75"],["B","95","60","70","80"]]} },
  { id:"risks",category:"Business judgment",type:"multi",selectMode:"open",prompt:"Which risks require mitigation before rollout?",instruction:"Select all the risks that are material and specific to this decision.",options:[["A","Recycled resin supply could be inconsistent in quality or availability"],["B","Retailers may not execute reverse logistics consistently"],["C","Consumers may not return packs despite the incentive"],["D","Recycled-resin pricing could rise if global demand for recycled feedstock increases"],["E","Retail price promotions could temporarily mask the packaging change's effect on sales"],["F","Competitors may use a different font"]],answer:["A","B","C"],points:10,exhibit:null },
...
const voltaRideQuestions = [
  ...
  { id: "risks", category: "Business judgment", type: "multi", selectMode: "open", prompt: "Which risks should VoltaRide prioritize before entering Surabaya?", instruction: "Select all the risks that are material and specific to this decision.", options: [["A", "Surabaya's two-wheeler licensing or municipal permit rules could cap or delay fleet deployment."], ["B", "Utilization could fall short of the 1.6 rides-per-scooter assumption used to size Plan D, lengthening payback."], ["C", "Incumbent operators could respond with fare cuts or charging-partner exclusivity, raising VoltaRide's effective acquisition cost."], ["D", "National smartphone ownership could decline over the next decade."], ["E", "A competitor's app icon may use a similar shade of green."], ["F", "Long-run battery-cell commodity prices could fall over the next decade, reducing future capex."]], answer: ["A", "B", "C"], points: 10, exhibit: null },
...
const javaMartQuestions = [
  ...
  { id: "breakeven", category: "Data interpretation", type: "single", prompt: "Which statement about the Match scenario's breakeven volume is correct?", instruction: "Use Exhibit 2. At the matched price (IDR 4,050, margin IDR 800 per unit), Java Mart would need 5.50m units just to equal Hold's IDR 4.4bn gross profit. Select the best-supported statement.", options: [["A", "Match's actual projected volume (4.375m units) falls short of the 5.50m units needed merely to equal Hold's gross profit, confirming Match is worse than Hold even on its own terms."], ["B", "Match's actual projected volume (4.375m units) exceeds the 5.50m breakeven volume, confirming Match outperforms Hold."], ["C", "Match's volume is irrelevant, since Hold's gross profit does not depend on any competitor response."], ["D", "Because 5.50m units is below the category's current 4.0m baseline, Match easily clears the breakeven bar."]], answer: "A", points: 10, exhibit: null },
  { id: "risks", category: "Business judgment", type: "multi", selectMode: "open", prompt: "Which risks should Java Mart prioritize before adopting the Differentiate response?", instruction: "Select all the risks that are material and specific to this decision.", options: [["A", "The loyalty/bundle program may not lift volume by the assumed amount if customers don't value the bundled offer."], ["B", "A 4% price cut may still not fully stop share loss to HematMart in the most price-sensitive segments."], ["C", "Suppliers may resist funding the loyalty program if it compresses their own trade terms."], ["D", "National retail electricity tariffs could rise over the next decade."], ["E", "A competitor's logo may use a similar shade of green."], ["F", "Long-run e-commerce penetration could reduce minimarket foot traffic over the next decade."]], answer: ["A", "B", "C"], points: 10, exhibit: null },
...
const nadiarMobileQuestions = [
  ...
  { id: "risks", category: "Business judgment", type: "multi", selectMode: "open", prompt: "Which risks should Nadiar prioritize before launching the new tier at IDR 30,000?", instruction: "Select all the risks that are material and specific to this decision.", options: [["A", "Subscribers may churn if the IDR 30,000 price is perceived as a premium versus competitors' entry tiers."], ["B", "Network costs could rise further if usage per subscriber exceeds the assumptions behind the cost exhibit."], ["C", "Competitor Z's faster relative share growth could erode Nadiar's lead if the trend continues."], ["D", "National smartphone import tariffs could change over the next decade."], ["E", "A competitor's logo may use a similar shade of green."], ["F", "Long-run satellite-internet adoption could reduce demand for terrestrial mobile data over the next decade."]], answer: ["A", "B", "C"], points: 10, exhibit: null },
...
function answerControl(q) {
  ...
  const counter = q.selectMode === "open" ? "" : `<div class="select-counter" data-required="${q.answer.length}">0 of ${q.answer.length} selected</div>`;
  return `${counter}<div class="choices multi">${options.map(([key, label]) => `<button class="choice" data-value="${key}"><i>${key}</i><span>${label}</span><b>✓</b></button>`).join("")}</div>`;
}
...
function wireQuestion(q) {
  ...
  const answerReady = () => {
    ...
    if (q.type === "multi") {
      if (q.selectMode === "open") return selected.length >= 1;
  ...
  if (q.type === "number" || q.type === "short") {
    ...
    document.querySelectorAll(".choice").forEach(button => button.onclick = () => {
      ...
      if (q.type === "single") {
        ...
        button.classList.toggle("selected");
        selected = [...document.querySelectorAll(".choice.selected")].map(x => x.dataset.value);
        const counter = document.querySelector(".select-counter");
        if (counter) {
          const required = Number(counter.dataset.required);
          counter.textContent = `${selected.length} of ${required} selected`;
          counter.classList.toggle("complete", selected.length === required);
        }
      }
      updateContinue();
...
function explain(id) {
  ...
  if (activeCase.id === "suryagrid") {
    const climateGuides = {
      ...
      score: "Portfolio A's weighted score = 100×50% + 100×25% + 40×15% + 70×10% = 88.0; Portfolio B's is 90×50%+80×25%+85×15%+90×10% = 86.75. A scores higher, but it breaches the IDR 170bn budget and +0.6% tariff guardrails (Exhibit 2), so the constraint-feasible Portfolio B should still be the recommendation.",
  ...
  if (activeCase.id === "javamart") {
    const responseGuides = {
      ...
      breakeven: "IDR 4.4bn ÷ IDR 800 per unit = 5.50m units needed to equal Hold's gross profit, but Match's actual projected volume is only 4.375m units (Exhibit 2) — short of breakeven, so Match underperforms Hold even on its own terms.",
  ...
  if (activeCase.id === "bumibite") {
    const bumiGuides = {
      ...
      profit: "Year 1 profit = 160.0 − 128.0 = IDR 32.0bn; Year 2 profit = 184.0 − 153.6 = IDR 30.4bn. Revenue grew 15.0% but operating costs grew 20.0%, so profit fell despite revenue growth.",
      ...
      valuation: "Original line synergy = (3,900 − 1,600 − 500 − 600 − 500) × 6.0m = IDR 4.2bn. Spicy line synergy = (3,900 − 1,800 − 550 − 600 − 500) × 4.0m = IDR 1.8bn. Combined synergy is 6.0bn; annual profit impact is 5.5 + 6.0 = IDR 11.5bn; at 8×, that is IDR 92.0bn.",
  ...
  const guides = {
    ...
    profit: "Year 1 profit = 200.0 − 160.0 = IDR 40.0bn; Year 2 profit = 230.0 − 194.8 = IDR 35.2bn. Revenue grew 15.0% but operating costs grew 21.75%, so profit fell despite revenue growth.",
    ...
    valuation: "RTD Coffee synergy = (4,800 − 2,000 − 500 − 700 − 600) × 6.0m = IDR 6.0bn. RTD Tea synergy = (4,800 − 2,200 − 600 − 700 − 600) × 3.0m = IDR 2.1bn. Combined synergy is 8.1bn; annual profit impact is 7.0 + 8.1 = IDR 15.1bn; at 8×, that is IDR 120.8bn.",
